...
export default function SatelliteTrackerDetails({ route, navigation }: any) {
  ...
  const [satellitePasses, setSatellitePasses] = useState<IssPass[]>([])
  ...
  useEffect(() => {
    const fetchSatelliteData = async () => {
      try {
        ...
        const tleResponse = await fetch(`${process.env.EXPO_PUBLIC_ASTROSHARE_API_URL}/satellite/tle?noradId=${noradId}`)
        ...
        console.log('Satellite Next Passes Data:', satelliteNextPassesData.passes);
        ...
        <ScrollView>
          <View style={satelliteTrackerStyles.content}>
            <View style={satelliteTrackerStyles.content.liveStats}>
              <Text style={satelliteTrackerStyles.content.liveStats.title}>{i18n.t('satelliteTracker.cssTracker.stats.title')}</Text>
              <DSOValues title={i18n.t('satelliteTracker.cssTracker.stats.latitude')} value={satelliteInfos ? satelliteInfos.currentPosition.satlatitude : <ActivityIndicator size={'small'} color={app_colors.white} animating />} />
              <DSOValues title={i18n.t('satelliteTracker.cssTracker.stats.longitude')} value={satelliteInfos ? satelliteInfos.currentPosition.satlongitude : <ActivityIndicator size={'small'} color={app_colors.white} animating />} />
              <DSOValues title={i18n.t('satelliteTracker.cssTracker.stats.altitude')} value={satelliteInfos ? `${satelliteInfos.currentPosition.sataltitude} Km` : <ActivityIndicator size={'small'} color={app_colors.white} animating />} />
              <DSOValues title={i18n.t('satelliteTracker.cssTracker.stats.country')} value={satelliteInfos ? `${getCountryByCode(satelliteInfos.currentCountry.country, currentLocale)} - ${getCountryFlag(satelliteInfos.currentCountry.country === i18n.t('satelliteTracker.cssTracker.stats.unknown') ? 'ZZ' : satelliteInfos.currentCountry.country)}` : <ActivityIndicator size={'small'} color={app_colors.white} animating />} />
            </View>
            <View style={satelliteTrackerStyles.content.nextPasses}>
              <Text style={satelliteTrackerStyles.content.nextPasses.title}>{i18n.t('satelliteTracker.cssTracker.nextPasses.title')}</Text>
              <Text style={satelliteTrackerStyles.content.nextPasses.subtitle}>{i18n.t('satelliteTracker.cssTracker.nextPasses.subtitle')}{currentUserLocation.common_name}</Text>
              <View style={satelliteTrackerStyles.content.nextPasses.container}>
...

                {
                  ...
                  (
                    ...
                    <Text style={satelliteTrackerStyles.content.nextPasses.noPasses}>{i18n.t('satelliteTracker.issTracker.nextPasses.noPasses')}</Text>
                }
            ...
            <View style={satelliteTrackerStyles.content.mapContainer}>
              <Text style={satelliteTrackerStyles.content.liveStats.title}>{i18n.t('satelliteTracker.issTracker.2dMap.title')}</Text>
              <Text style={satelliteTrackerStyles.content.liveStats.subtitle}>{i18n.t('satelliteTracker.issTracker.2dMap.subtitle')}</Text>
