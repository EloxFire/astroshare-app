...
export default function PlanetDetails({ route, navigation }: any) {
  ...
  const planet: (Planet & EclipticCoordinate & EquatorialCoordinate & HorizontalCoordinate) = params.planet;
    ...
    <View style={globalStyles.body}>
      ...
      <ScrollView>
        ...
        <View style={objectDetailsStyles.content.dsoInfos}>
          ...
          <DSOValues title="Position" value={planet.isInferior ? "Avant la Terre" : "Après la Terre"} />
          <DSOValues title="Inclinaison" value={(23.5 + parseFloat(planet.i.toFixed(1))).toString() + "°"} />
          <DSOValues title="Masse" value={planet.i.toFixed(2) + "x Terre"} />
          <DSOValues title="Période orbitale" value={planet.T + " années"} />
          <DSOValues title="Distance" value={planet.a.toFixed(2) + "M Km"} />
